...
using Diderot.Knapsack: Instance, InOrder, ByWeightDecr, RestrictLowDist, RelaxLowCap
...
@testset "model methods" begin
    ...
    @test Diderot.next_variable(inst, dd, InOrder()) == 1
...
@testset "top-down decision diagrams" begin
    inst = Instance([4.0, 3.0, 2.0], [3, 2, 2], 4)
    dd = Diderot.DecisionDiagram(inst)
    Diderot.top_down!(dd, inst, InOrder())
...
@testset "longest path" begin
    ...
    Diderot.top_down!(dd2, inst2, ByWeightDecr())
...
@testset "restriction" begin
    ...
    @testset "width 1" begin
        dd = Diderot.DecisionDiagram(inst)
        Diderot.top_down!(dd, inst, InOrder(),
                          process_layer=RestrictLowDist(1))
        @test length(dd.layers) == 4
    ...
    @testset "width 2" begin
        dd = Diderot.DecisionDiagram(inst)
        Diderot.top_down!(dd, inst, InOrder(),
                          process_layer=RestrictLowDist(2))
        @test length(dd.layers) == 4
...
@testset "relaxation" begin
    ...
    @testset "width 1" begin
        dd = Diderot.DecisionDiagram(inst)
        Diderot.top_down!(dd, inst, InOrder(), process_layer=RelaxLowCap(1))
        @test length(dd.layers) == 4
        @test all(l -> length(l) == 1, dd.layers)
    ...
    @testset "width 2" begin
        dd = Diderot.DecisionDiagram(inst)
        Diderot.top_down!(dd, inst, InOrder(), process_layer=RelaxLowCap(2))
        @test length(dd.layers) == 4
        @test all(l -> length(l) in (1, 2), dd.layers)
...
@testset "branch and bound" begin
    ...
    @testset "width 1" begin
        sol = Diderot.branch_and_bound(inst, InOrder(), RestrictLowDist(1),
                                       RelaxLowCap(2))
        @test sol.decisions == [false, true, true]
        @test sol.objective ≈ 5.0
    end
    ...
    @testset "width 2" begin
        sol = Diderot.branch_and_bound(inst, InOrder(), RestrictLowDist(2),
                                       RelaxLowCap(2))
        @test sol.decisions == [false, true, true]
        @test sol.objective ≈ 5.0
    end
    ...
    @testset "width 3" begin
        sol = Diderot.branch_and_bound(inst, InOrder(), RestrictLowDist(3),
                                       RelaxLowCap(3))
        @test sol.decisions == [false, true, true]
        @test sol.objective ≈ 5.0
    end
    ...
    @testset "n 5, width 2" begin
        ...
        sol = Diderot.branch_and_bound(inst2, InOrder(), RestrictLowDist(2),
                                       RelaxLowCap(2))
